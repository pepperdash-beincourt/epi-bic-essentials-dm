...
namespace PepperDash.Essentials.DM.Routing
{
    ...
    /// feedback. Unlike the chassis-specific <see cref="DmMatrixOutput"/>/<see cref="DmMatrixInput"/>
    /// slots (wired directly to Crestron <c>CardDevice</c> events), this derives slots and
    /// per-signal-type route state from the port collections plus route-change notifications the
    /// controller already raises, so any port-based <see cref="IRoutingMidpointWithFeedback"/>
    /// matrix can expose named slots without bespoke hardware wiring. Pure Essentials.Core routing
    /// types, so it is unit-testable without a processor.
    /// </summary>
    public sealed class RoutingPortNamedSlots
    {
        ...
        /// <summary>
        /// Captures the port collections. Slots are built lazily on first access (with 1-based slot
        /// numbers in enumeration order) so the controller can construct this before its ports are
        /// fully populated - the build snapshots the final port state at first runtime use.
        /// </summary>
    ...
    public sealed class RoutingPortOutputSlot : IRoutingOutputSlotInfo
    {
        ...
        /// components so per-signal-type consumers resolve each independently, mirroring the DM chassis
        /// slot model. Raises <see cref="OutputSlotChanged"/> only when something actually changed.
        /// </summary>

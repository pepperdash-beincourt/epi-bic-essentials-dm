...
| Source file | [`src/Chassis/HdMdNxM4kEController.cs`](../../../src/Chassis/HdMdNxM4kEController.cs) |
| Language | C# |
| Declaration | `class HdMdNxM4kEController` with declared base/contract list `CrestronGenericBaseDevice, IRoutingMidpointWithFeedback` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.DM.Chassis` |

## What

`HdMdNxM4kEController` is a source-level implementation type with responsibilities defined by its members and inheritance. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `CrestronGenericBaseDevice, IRoutingMidpointWithFeedback`. Public methods declared in this source file include: `ExecuteSwitch`, `ClearRoute`, `GetController`, `BuildDevice`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
...
Before proposing a change, read this declaration, its full source file, all repository references to `HdMdNxM4kEController`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.

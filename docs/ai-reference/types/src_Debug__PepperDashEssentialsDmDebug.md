...
| Source file | [`src/Debug.cs`](../../../src/Debug.cs) |
| Language | C# |
| Declaration | `class PepperDashEssentialsDmDebug` |
| Accessibility | `internal` |
| Namespace/module | `Global namespace` |

## What

`PepperDashEssentialsDmDebug` is a source-level implementation type with responsibilities defined by its members and inheritance. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Public methods declared in this source file include: `ResetDebugLevels`, `SetDebugLevels`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
...
Before proposing a change, read this declaration, its full source file, all repository references to `PepperDashEssentialsDmDebug`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.

...
| Source file | [`src/Endpoints/Transmitters/DmTxHelpers.cs`](../../../src/Endpoints/Transmitters/DmTxHelpers.cs) |
| Language | C# |
| Declaration | `class DmTxControllerFactory` with declared base/contract list `EssentialsPluginDeviceFactory<DmTxControllerBase>` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.DM` |

## What

`DmTxControllerFactory` is a construction boundary that maps configuration or discovery inputs into concrete objects. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `EssentialsPluginDeviceFactory<DmTxControllerBase>`. Public methods declared in this source file include: `GetDmTxForChassisWithoutIpId`, `GetDmTxForChassisWithIpId`, `GetDmTxController`, `SetPortHdcpCapability`, `ClearRoute`, `BuildDevice`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
...
Before proposing a change, read this declaration, its full source file, all repository references to `DmTxControllerFactory`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.

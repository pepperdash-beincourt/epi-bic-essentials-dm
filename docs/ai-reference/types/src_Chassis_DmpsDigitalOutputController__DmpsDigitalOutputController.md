...
| Source file | [`src/Chassis/DmpsDigitalOutputController.cs`](../../../src/Chassis/DmpsDigitalOutputController.cs) |
| Language | C# |
| Declaration | `class DmpsDigitalOutputController` with declared base/contract list `Device, IRoutingMidpointWithFeedback, IHasFeedback` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.DM` |

## What

`DmpsDigitalOutputController` is a device-model type that owns lifecycle, controls, feedback, or communication responsibilities. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Before proposing a change, read this declaration, its full source file, all repository references to `DmpsDigitalOutputController`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.

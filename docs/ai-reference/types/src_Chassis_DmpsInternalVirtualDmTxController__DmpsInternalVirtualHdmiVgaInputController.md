...
| Source file | [`src/Chassis/DmpsInternalVirtualDmTxController.cs`](../../../src/Chassis/DmpsInternalVirtualDmTxController.cs) |
| Language | C# |
| Declaration | `class DmpsInternalVirtualHdmiVgaInputController` with declared base/contract list `Device, ITxRouting, IHasFeedback` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.DM` |

## What

`DmpsInternalVirtualHdmiVgaInputController` is a device-model type that owns lifecycle, controls, feedback, or communication responsibilities. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `Device, ITxRouting, IHasFeedback`. Public methods declared in this source file include: `SetPortHdcpCapability`, `AddToFeedbackList`, `ExecuteNumericSwitch`, `ExecuteSwitch`, `ClearRoute`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
...
Before proposing a change, read this declaration, its full source file, all repository references to `DmpsInternalVirtualHdmiVgaInputController`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.

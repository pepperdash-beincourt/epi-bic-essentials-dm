...
| Source file | [`src/IDmSwitch.cs`](../../../src/IDmSwitch.cs) |
| Language | C# |
| Declaration | `interface IDmSwitchWithEndpointOnlineFeedback` with declared base/contract list `IDmSwitch` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.DM` |

## What

`IDmSwitchWithEndpointOnlineFeedback` is a contract that callers and implementations use to agree on behavior. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `IDmSwitch`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
...
Before proposing a change, read this declaration, its full source file, all repository references to `IDmSwitchWithEndpointOnlineFeedback`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.

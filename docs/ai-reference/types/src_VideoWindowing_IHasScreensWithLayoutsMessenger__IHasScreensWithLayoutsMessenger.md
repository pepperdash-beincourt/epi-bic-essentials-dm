# IHasScreensWithLayoutsMessenger
...
| Language | C# |
| Declaration | `class IHasScreensWithLayoutsMessenger` with declared base/contract list `MessengerBase` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.DM.VideoWindowing` |

## What

`IHasScreensWithLayoutsMessenger` is a message-handling boundary that translates between transport payloads and application behavior. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `MessengerBase`. Public methods declared in this source file include: `SendCurrentLayoutStatus`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.

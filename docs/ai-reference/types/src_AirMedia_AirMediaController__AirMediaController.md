...
| Language | C# |
| Declaration | `class AirMediaController` with declared base/contract list `CrestronGenericBridgeableBaseDevice, IRoutingMidpointWithFeedback, IIROutputPorts, IComPorts` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.DM.AirMedia` |

## What

`AirMediaController` is a source-level implementation type with responsibilities defined by its members and inheritance. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.

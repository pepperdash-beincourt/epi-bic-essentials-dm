...
| Source file | [`src/VideoStatusHelpers.cs`](../../../src/VideoStatusHelpers.cs) |
| Language | C# |
| Declaration | `class VideoStatusHelper` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.DM` |

## What

`VideoStatusHelper` is a source-level implementation type with responsibilities defined by its members and inheritance. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Public methods declared in this source file include: `GetHdmiInputStatusFuncs`, `GetVgaInputStatusFuncs`, `GetBncInputStatusFuncs`, `GetDmInputStatusFuncs`, `GetDisplayPortInputStatusFuncs`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.

...
Public methods declared in this source file include: `Assembly_Loads_Successfully`, `Assembly_Name_Matches_Expected`, `All_Factory_Types_Are_Discoverable`, `Factory_Count_Matches_Expected`, `Factory_Exists_ByName`, `All_Factories_Have_Parameterless_Constructor`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.

...
| Source file | [`src/Config/HdPsXxxPropertiesConfig.cs`](../../../src/Config/HdPsXxxPropertiesConfig.cs) |
| Language | C# |
| Declaration | `class HdPsXxxPropertiesConfig` |
| Accessibility | `public` |
| Namespace/module | `PepperDash_Essentials_DM.Config` |

## What

`HdPsXxxPropertiesConfig` is a configuration or data-transfer shape used to deserialize and validate structured input. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Before proposing a change, read this declaration, its full source file, all repository references to `HdPsXxxPropertiesConfig`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.

...
| Source file | [`src/Endpoints/Receivers/DmRmc4kZScalerCController.cs`](../../../src/Endpoints/Receivers/DmRmc4kZScalerCController.cs) |
| Language | C# |
| Declaration | `class DmRmc4kZScalerCController` with declared base/contract list `DmRmcControllerBase, IRmcRoutingWithFeedback,` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.DM` |

## What

`DmRmc4kZScalerCController` is a source-level implementation type with responsibilities defined by its members and inheritance. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `DmRmcControllerBase, IRmcRoutingWithFeedback,`. Public methods declared in this source file include: `ClearRoute`, `LinkToApi`, `ExecuteSwitch`, `ExecuteNumericSwitch`, `SetDmInHdcpState`, `SetHdmiInHdcpState`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
...
Before proposing a change, read this declaration, its full source file, all repository references to `DmRmc4kZScalerCController`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.

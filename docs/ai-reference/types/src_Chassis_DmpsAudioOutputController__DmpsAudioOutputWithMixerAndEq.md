...
| Source file | [`src/Chassis/DmpsAudioOutputController.cs`](../../../src/Chassis/DmpsAudioOutputController.cs) |
| Language | C# |
| Declaration | `class DmpsAudioOutputWithMixerAndEq` with declared base/contract list `DmpsAudioOutputWithMixer` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.DM` |

## What

`DmpsAudioOutputWithMixerAndEq` is a source-level implementation type with responsibilities defined by its members and inheritance. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `DmpsAudioOutputWithMixer`. Public methods declared in this source file include: `LinkToApi`, `RecallEqPreset`, `GetVolumeMin`, `GetVolumeMax`, `RecallPreset`, `RecallStartupVolume`, `SetVolumeScaled`, `ScaleVolumeFeedback`, `SendScaledVolume`, `SetVolume`, `MuteOn`, `MuteOff`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
...
Before proposing a change, read this declaration, its full source file, all repository references to `DmpsAudioOutputWithMixerAndEq`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.

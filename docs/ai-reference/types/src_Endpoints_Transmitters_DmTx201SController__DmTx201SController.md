...
| Source file | [`src/Endpoints/Transmitters/DmTx201SController.cs`](../../../src/Endpoints/Transmitters/DmTx201SController.cs) |
| Language | C# |
| Declaration | `class DmTx201SController` with declared base/contract list `DmTxControllerBase, ITxRoutingWithFeedback, IHasFreeRun, IVgaBrightnessContrastControls` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.DM` |

## What

`DmTx201SController` is a source-level implementation type with responsibilities defined by its members and inheritance. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `DmTxControllerBase, ITxRoutingWithFeedback, IHasFreeRun, IVgaBrightnessContrastControls`. Public methods declared in this source file include: `LinkToApi`, `SetFreeRunEnabled`, `SetVgaBrightness`, `SetVgaContrast`, `ExecuteNumericSwitch`, `ExecuteSwitch`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
...
Before proposing a change, read this declaration, its full source file, all repository references to `DmTx201SController`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.

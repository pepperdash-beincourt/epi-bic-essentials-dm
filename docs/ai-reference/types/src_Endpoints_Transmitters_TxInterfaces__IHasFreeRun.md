...
| Language | C# |
| Declaration | `interface IHasFreeRun` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.DM` |

## What

`IHasFreeRun` is a contract that callers and implementations use to agree on behavior. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Before proposing a change, read this declaration, its full source file, all repository references to `IHasFreeRun`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.

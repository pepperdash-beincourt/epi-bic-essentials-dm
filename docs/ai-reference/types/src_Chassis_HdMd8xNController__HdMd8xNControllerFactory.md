...
| Source file | [`src/Chassis/HdMd8xNController.cs`](../../../src/Chassis/HdMd8xNController.cs) |
| Language | C# |
| Declaration | `class HdMd8xNControllerFactory` with declared base/contract list `EssentialsPluginDeviceFactory<HdMd8xNController>` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.DM.Chassis` |

## What

`HdMd8xNControllerFactory` is a construction boundary that maps configuration or discovery inputs into concrete objects. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `EssentialsPluginDeviceFactory<HdMd8xNController>`. Public methods declared in this source file include: `ClearRoute`, `AddFeedbackCollections`, `AddCollectionsToList`, `AddCollectionToList`, `AddFeedbackToList`, `ExecuteSwitch`, `ExecuteNumericSwitch`, `LinkToApi`, `BuildDevice`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
...
Before proposing a change, read this declaration, its full source file, all repository references to `HdMd8xNControllerFactory`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.
